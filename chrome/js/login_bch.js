...
function doauthen() {
    ...
    if (checkForm() == true) {
	 ...
	 //勾选系统配置--服务配置--域名就将用户名拼接成用户名@服务名
        ...
        var password = encodeURIComponent(encodeURIComponent(document.getElementById("pwd").value));
        var service = "";
        ...
        AuthInterFace.init("http://210.77.16.21/eportal/");
        AuthInterFace.login(userId,password,service,queryString,operatorPwd,code,function(authResult){
        	 if(authResult.result=="success"){
               ...
               if(successPage&&successPage!=''&&successPage=="true"){
                  ...
                  AuthInterFace.getOnlineUserInfo(authResult.userIndex,function(onlineUserInfo){
                       ...
                       if (userUrl && userUrl != "" && userUrl != "http://123.123.123.123/") {
...
function checkForm() {
    var a = $("#username").attr("value").split(" ");
    if (a.length != 1) {
        showerror("\u7528\u6237\u540d\u4e0d\u80fd\u542b\u6709\u7a7a\u683c\uff01", 1);
        return false
    }
...
function freshImage() {
	jQuery("#validImage").attr("src", "http://210.77.16.21/eportal/validcode?rnd=?" + Math.random());
}
...
function initInputFromCookies_hk() {
    ...
    if(coolieoperatorpwd!=null&&coolieoperatorpwd!=''&&document.getElementById("isNoOperatorPasswordFrameId"))
		{
